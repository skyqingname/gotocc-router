...
自有邀请返佣按本次入账的站内 USD 代币计算，不计算法币汇率；可返佣入口、三代比例和迁移行为见[邀请关系与三代返佣](AFFILIATE.md)。
...
> **注意**：旧版外部支付服务中的历史订单数据不会自动迁移。建议保留原服务一段时间以便查询历史记录。
