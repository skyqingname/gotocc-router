...
```text
Git/GitHub: v0.1.183+custom.007
GHCR:       ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.007
```
...
```bash
curl -sSL https://raw.githubusercontent.com/LuckyKuang/sub2api-plus/main/deploy/install.sh | sudo bash -s -- install --version 'v0.1.183+custom.007'
```
...
```bash
sudo ./install.sh install --version 'v0.1.183+custom.007'
```

Roll back a downloaded-script installation one operation at a time:

```bash
sudo ./install.sh rollback 'v0.1.183+custom.003'
```
