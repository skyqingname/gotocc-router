...
Apple/OCI image:    ghcr.io/luckykuang/sub2api-plus:v0.1.176-custom.003
```
...
```bash
docker build \
  --build-arg VERSION=0.1.176+custom.003 \
  --tag ghcr.io/luckykuang/sub2api-plus:v0.1.176-custom.003 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:v0.1.176-custom.003`. Until then, keep
