...
Apple/OCI image:    ghcr.io/luckykuang/sub2api-plus:v0.1.178-custom.002
```
...
```bash
docker build \
  --build-arg VERSION=0.1.178+custom.002 \
  --tag ghcr.io/luckykuang/sub2api-plus:v0.1.178-custom.002 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:v0.1.178-custom.002`. Until then, keep
