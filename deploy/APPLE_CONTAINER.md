...
Apple/OCI image:    ghcr.io/skyqingname/sub2api-plus:v0.2.4-custom.006
```
...
docker build \
  ...
  --tag ghcr.io/skyqingname/sub2api-plus:v0.2.4-custom.006 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/skyqingname/sub2api-plus:v0.2.4-custom.006`. Until then, keep
