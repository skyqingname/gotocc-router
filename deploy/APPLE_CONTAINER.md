...
Apple/OCI image:    ghcr.io/skyqingname/sub2api-plus:v0.2.1-custom.003
```
...
docker build \
  ...
  --tag ghcr.io/skyqingname/sub2api-plus:v0.2.1-custom.003 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/skyqingname/sub2api-plus:v0.2.1-custom.003`. Until then, keep
