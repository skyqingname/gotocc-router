...
Apple/OCI image:    ghcr.io/skyqingname/sub2api-plus:v0.2.4-custom.008
```
...
```bash
docker build \
  --build-arg VERSION=0.2.4+custom.008 \
  --tag ghcr.io/skyqingname/sub2api-plus:v0.2.4-custom.008 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/skyqingname/sub2api-plus:v0.2.4-custom.008`. Until then, keep
