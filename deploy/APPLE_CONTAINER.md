...
Application:        0.1.183+custom.007
Apple/OCI image:    ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.007
```
...
```bash
docker build \
  --build-arg VERSION=0.1.183+custom.007 \
  --tag ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.007 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.007`. Until then, keep
