...
```text
Git/GitHub:         v0.1.183+custom.008
Application:        0.1.183+custom.008
Apple/OCI image:    ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.008
```
...
```bash
docker build \
  --build-arg VERSION=0.1.183+custom.008 \
  --tag ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.008 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.008`. Until then, keep
