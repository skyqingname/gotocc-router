...
describe('Prompt Audit components', () => {
  ...
  it('supports group search, stale configured groups, nine scanners, and bounded worker inputs', async () => {
    const draft: PromptAuditDraft = {
      ...
      worker_count: 4, queue_capacity: 100, audit_prompt: 'audit prompt', default_audit_prompt: 'default audit prompt', scanners: SCANNER_CATALOG.map((item) => item.id), all_groups: false, group_ids: [1, 99],
