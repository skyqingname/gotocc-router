      ...
      >
        <div class="py-1">
          <template v-if="account">
            <button v-if="account.platform !== 'video'" @click="$emit('test', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
            ...
            <button v-if="account.platform !== 'video'" @click="$emit('schedule', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="clock" size="sm" class="text-orange-500" />
              {{ t('admin.scheduledTests.schedule') }}
            </button>
            <button @click="$emit('usage-alert', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="bell" size="sm" class="text-green-600" />
              {{ t('admin.usageAlert.menu') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'usage-alert', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
