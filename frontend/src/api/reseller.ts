...
export interface ResellerProfile { user_id: number; enabled: boolean; invitation_code: string; default_multiplier: number; rebate_rates: (number | null)[]; global_rebate_rates: number[] }
...
export const resellerAPI = {
  ...
  adminSave: async (id: number, enabled: boolean, rebate_rates: (number|null)[]) => (await apiClient.put<ResellerProfile>(`/admin/users/${id}/reseller`, { enabled, rebate_rates })).data
}
