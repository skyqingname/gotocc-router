export default {
  promptAudit: {
    ...
    textTest: {
      "title": "Text test",
      "description": "Enter text to evaluate it with the saved audit policy.",
      "savedPolicy": "Saved policy v{version}",
      "dirty": "There are unsaved changes. This test uses the last saved policy.",
      "scope": "This calls the audit endpoint only. It does not invoke a business model or create a user audit event. Testing works while auditing is off; live blocking also depends on mode and group scope.",
      "input": "Text to audit",
      ...
      "waitingTitle": "See the audit decision",
      "waitingBody": "Submit text to see the blocking decision, score and reason.",
      "allow": "Pass",
      "flag": "Flagged",
      "block": "Would block",
      "failed": "Call failed",
      ...
      "reason": "Reason",
      "noReason": "The audit model did not provide a reason",
      "latency": "Elapsed",
      "chunks": "Total chunks",
      "node": "Audit endpoint",
      "mode": "Current mode",
      ...
      "httpStatus": "Endpoint HTTP status",
    ...
    pool: {
      ...
      secretHint: 'Plaintext exists only in this editor and is cleared immediately after a successful save.', clearSecret: 'Explicitly clear the saved API Key', timeout: 'Per endpoint call timeout (ms)', inputLimit: 'Unicode characters per chunk',
      timeoutRange: 'Supported range: {min}–{max} ms.', inputLimitRange: 'Supported range: {min}–{max} Unicode characters.', inputLimitBehavior: 'A prioritized segment above this value is split further; the latest input is submitted separately from history. To submit one message intact, set the limit no lower than its character count and ensure the model context is sufficient. When several nodes are enabled, the smallest limit applies.',
    ...
    auditPrompt: {
      ...
      hint: 'After saving, the same prompt is used by async workers, synchronous blocking, and model probes. Audit logs record only its hash, never its content.',
      required: 'The audit prompt cannot be empty or exceed 20,000 Unicode characters.',
