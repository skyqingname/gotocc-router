...
// HasReusableInvitationCodeUses applies the HasEdge predicate on the "reusable_invitation_code_uses" edge.
func HasReusableInvitationCodeUses() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasReusableInvitationCodeUsesWith applies the HasEdge predicate on the "reusable_invitation_code_uses" edge with a given conditions (other predicates).
func HasReusableInvitationCodeUsesWith(preds ...predicate.ReusableInvitationCodeUse) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newReusableInvitationCodeUsesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
