...
func TestReconcileCRSOllamaCloudUsageExtraDropsRetiredBillingProbeKeys(t *testing.T) {
	remote := map[string]any{
		"crs_account_id":                     "remote-1",
		"upstream_billing_probe_enabled":     true,
		"upstream_billing_rate_sync_enabled": true,
		"upstream_billing_probe":             map[string]any{"status": "remote"},
	}
	extra := mergeMap(nil, remote)
	reconcileCRSOllamaCloudUsageExtra(nil, PlatformOpenAI, AccountTypeAPIKey, map[string]any{"api_key": "new"}, extra)
	require.NotContains(t, extra, "upstream_billing_probe_enabled")
	require.NotContains(t, extra, "upstream_billing_rate_sync_enabled")
