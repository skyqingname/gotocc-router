...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string) (string, *User, error) {
	...
	// 一次性邀请码保持 Plus 既有的注册后 fail-open 消费语义。可复用邀请码
	// 已在用户创建事务内完成强一致消费。
	if registrationInvitation != nil && registrationInvitation.redeem != nil {
		_ = s.useRegistrationInvitation(ctx, registrationInvitation, user, "email", true)
	}
	// 应用优惠码（如果提供且功能已启用）
...
func (s *AuthService) createUserWithRegistrationInvitation(ctx context.Context, user *User, invitation *registrationInvitation, authSource string) error {
	if invitation == nil || invitation.reusable == nil {
