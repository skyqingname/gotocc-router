...
CREATE TABLE IF NOT EXISTS groups_video_price_backup_218 AS
...
COMMENT ON TABLE groups_video_price_backup_218 IS
    '迁移 218 清空非 Grok/非 composite 分组视频价前的快照。composite 可能路由到 Grok 账号，予以保留。确认无需回滚后可安全 DROP；回滚方式：UPDATE groups g SET video_price_480p = b.video_price_480p, ... FROM groups_video_price_backup_218 b WHERE g.id = b.group_id';
