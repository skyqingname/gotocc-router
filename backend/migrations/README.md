...
unchanged because filenames and checksums are already deployed. A migration
imported from a reviewed production lineage may retain its deployed filename
only when `tools/check_new_migrations.py` binds that exact path and SHA-256.
Do not add new duplicates.
