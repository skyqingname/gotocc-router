...
Video 配置入口修正：协议与参数移入分组编辑；导入固定影策源码的全部 39 个视频 provider 及其声明式请求、查询和结果引擎。支持协议扩展参数/接口定义覆盖、Google/TC3/火山签名，以及 Video 账号 Secret Key 脱敏编辑。Migration 278 复制原渠道模型配置到分组；新任务保存完整协议快照，旧任务保持原配置。新增 singular video/generations 别名，并补齐根路径及 Codex 直连路由的站长价格上下文。旧程序不理解新分组/任务协议，不能仅回滚二进制。未逐家调用付费上游，不将目录导入等同于真实生成验收。
